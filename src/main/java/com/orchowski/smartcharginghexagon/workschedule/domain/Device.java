...
@Getter
@Setter//[Question] HOW TO GET RID OF PUBLIC SETTERS IN THIS ARCH???????????????????, is creating new Dto/value objects on port level is ok? There will be 4 objects before returning in restcontroller Entity -> Domain object -> domain object dto -> rest controller dto
public class Device {
